...
* 字典：
  - ht 属性是包含_两个_项的数组，字典只使用 `ht[0]`，`ht[1]` 只会在对 ht[0] `rehash` 时使用;
...
* 解决冲突：`链地址法`——多个哈希节点构成一个_单向链表_；为了速度考虑，程序总是将新节点添加到链表的`表头`。
...
哈希表扩展和收缩：
* 扩展
  - 服务器_没有执行_ `BGSAVE` 或 `BGREWRITEAOF`，并且负载因子`大于等于 1`；
  - 服务器_正在执行_ `BGSAVE` 或 `BGREWRITEAOF`，并且负载因子`大于等于 5`。
* 收缩：负载因子`小于 0.1`。
