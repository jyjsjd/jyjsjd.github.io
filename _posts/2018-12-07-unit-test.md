...
* `org.springframework.test.web.servlet.request.MockMvcRequestBuilders.*`：提供 HTTP 请求方法。
...
* 使用 `@BeforeClass`、`@AfterClass`等方法保证测试前后数据库状态一致。
...
表结构统一放入 `schema.sql`，数据放入 `data.sql`，测试启动时内存数据库会自动加载文件内容。
